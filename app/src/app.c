...
void update_logic(App* app) {
    if (app->input.mouse_left_click && !app->scene.is_cube_mode && !app->scene.cube.is_animating) {
        ...
        if (picked >= 0) {
            ...
            app->scene.orbit_distance = sqrt(
                ...
                app->player.camera.position.y * app->player.camera.position.y +
    ...
    if (!app->scene.is_cube_mode) {
        ...
        if (app->input.mouse_right_pressed) {
            ...
            app->player.camera.position.y = -app->player.camera.front.y * app->scene.orbit_distance;
...
void app_init(App* app) {
    ...
    app->window = SDL_CreateWindow("Rubik's Sanctuary", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 800, 600, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN);
    ...
    app->scene.metal_texture_id = load_texture("assets/textures/metal.jpg");
    ...
    if (load_model(&(app->scene.lamp_model), "assets/models/lamp.obj") == 0) {
        printf("Lampa sikeresen betoltve. Csucspontok: %d\n", app->scene.lamp_model.n_vertices);
        scale_model(&(app->scene.lamp_model), 1.0, 1.0, 1.0); 
    } else {
        printf("HIBA: Nem sikerult betolteni a lampat!\n");
    }
...
void app_run(App* app) {
    while (app->is_running) {
        ...
        if (app->input.keyboard_state[SDL_SCANCODE_P]) {
